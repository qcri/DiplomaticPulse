...
class TestBeautifulParser(unittest.TestCase):
    """
    Class containing the test suite for html_div_bs4().

    Tests are programmed as prescribed the pythons unittest's package.

    """

    def test_html_div_bs4(self):
        """
        We pass url to Beautifullsoup object to html_a_span_bs4 and expect it to return text.
        """
        html = "<a><div>The MFA is pleased to announce the launching of its new website</div><a>"
        html = BeautifulSoup(html, "html.parser")
        result = util .html_div_bs4(html)
