...
class TestBeautifulParser(unittest.TestCase):
    """
    Class containing the test suite for html_strong_bs4(().

    Tests are programmed as prescribed the pythons unittest's package.

    """

    def test_html_strong_bs4(self):
        """
        We pass url to Beautifullsoup object to html_strong_bs4( and expect it to return text.
        """
        html = "<strong>The MFA is pleased to announce the launching of its new website<strong>"
        html = BeautifulSoup(html, "html.parser")
        result = util.html_strong_bs4(html)
